// ==========================================================
// 🧠 LIORA — CORE v70-E
// - Tema: igual v62 (plano + sessões completas)
// - Upload: Modelo D (outline + sessões a partir do PDF)
// - Corrige títulos duplicados e hover/click em ambas modalidades
// ==========================================================

(function () {
  console.log("🔵 Inicializando Liora Core v70-E...");
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    // --------------------------------------------------------
    // TEMA: PLANO DE SESSÕES (igual v62)
    // --------------------------------------------------------
   ...
   {"q": "outra pergunta de revisão rápida", "a": "resposta direta"}
    ...
    function renderPlanoResumo(plano) {
      ...
      plano.forEach((p, index) => {
        ...
        // ❗ CORREÇÃO DE TÍTULO DUPLICADO (upload)
        div.textContent = p.titulo || p.nome || `Sessão ${index + 1}`;

        // hover igual para tema e upload
    ...
    // --------------------------------------------------------
    // FLUXO: GERAR POR TEMA (como v62, com cache)
    // --------------------------------------------------------
    ...
    async function fluxoUpload(file, nivel) {
      ...
      try {
        ...
        // compatível com extrairBlocos OU extrairBlocosDoPDF
        const extrairFn =
    ...
    console.log("🟢 Liora Core v70-E carregado com sucesso");
  });
})();
