// ==========================================================
// 🧠 LIORA — CORE v70-B
// - Tema: igual v62 (plano + sessões tipo aula)
// - Upload PDF: pipeline novo (blocos → seções → outline → sessões Modelo D)
// - Compatível com semantic.js v40 e outline-generator.js (Modelo D)
// ==========================================================

(function () {
  console.log("🔵 Inicializando Liora Core v70-B...");

  document.addEventListener("DOMContentLoaded", () => {
    // --------------------------------------------------------
    // ELEMENTOS (tudo alinhado com seu HTML)
    // --------------------------------------------------------
    const els = {
      // modos
    ...
    // --------------------------------------------------------
    // LLM CALL (back-end /api/liora) + exposição global
    // --------------------------------------------------------
    ...
    // para semantic/outline usarem
    window.callLLM = callLLM;

    // --------------------------------------------------------
    // TEMA: PLANO DE SESSÕES (mesmo conceito do v62)
    // --------------------------------------------------------
...

{
 ...
 "flashcards": [
   ...
   {"q": "outra pergunta de revisão rápida", "a": "resposta direta"}
    ...
    // --------------------------------------------------------
    // FLUXO: GERAR POR TEMA (igual v62, com cache)
    // --------------------------------------------------------
    ...
    // --------------------------------------------------------
    // FLUXO: GERAR POR UPLOAD (PDF) — Modelo D
    // --------------------------------------------------------
    async function fluxoUpload(file, nivel) {
      ...
      try {
        ...
        if (!window.LioraPDFExtractor || !window.LioraPDF) {
          throw new Error("Módulos de PDF não encontrados (LioraPDFExtractor / LioraPDF).");
        }
        if (!window.LioraOutline) {
          throw new Error("Módulo LioraOutline não encontrado.");
        }

        // 1) Extrair blocos
        const blocos = await window.LioraPDFExtractor.extrairBlocos(file);
        ...
        atualizarStatus("upload", "🧩 Gerando outline...", 40);

        // 3) Outlines por seção
        const outlines = await window.LioraOutline.gerarOutlinesPorSecao(secoes);
        console.log("🧠 Outlines por seção:", outlines);

        // 4) Outline unificado
        const outlineUnico = window.LioraOutline.unificarOutlines(outlines);
        console.log("🧠 Outline unificado:", outlineUnico);

        atualizarStatus("upload", "📚 Gerando sessões...", 70);

        // 5) Plano final (Modelo D)
        const planoFinal = await window.LioraOutline.gerarPlanoDeEstudo(outlineUnico);
    ...
    if (els.inpFile) {
      els.inpFile.addEventListener("change", e => {
        ...
        const label = document.getElementById("upload-text") ||
          document.querySelector("[data-upload-label]");
        if (label) {
    ...
    console.log("🟢 Liora Core v70-B carregado com sucesso");
  });
})();
