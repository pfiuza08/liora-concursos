...

{
 ...
 "flashcards": [
   ...
   {"q": "outra pergunta de revisão rápida", "a": "resposta direta"}
    ...
    // --------------------------------------------------------
    // UPLOAD PDF: DETECÇÃO DE CAPÍTULOS
    // --------------------------------------------------------
    ...
    // --------------------------------------------------------
    // UPLOAD PDF: GERAÇÃO DE SESSÃO POR CAPÍTULO
    // --------------------------------------------------------
    async function gerarSessaoPorCapitulo(capitulo, numero, total) {
      const prompt = `
Você é Liora, especialista em educação.

Use EXCLUSIVAMENTE o conteúdo abaixo (título + texto) para montar uma sessão de estudo fiel ao capítulo.

TÍTULO DO CAPÍTULO:
${capitulo.titulo}

TRECHO DO CAPÍTULO (pode estar parcial):
${capitulo.conteudo.join("\n")}

Gere APENAS JSON com a estrutura:

{
 "titulo": "Sessão ${numero} — ${capitulo.titulo}",
 "objetivo": "objetivo de aprendizagem baseado SÓ neste capítulo",
 "conteudo": {
   "introducao": "resuma o capítulo em 2-3 parágrafos introdutórios",
   "conceitos": [
     "conceito importante do capítulo explicado de forma fiel",
     "outro conceito importante",
     "outro conceito, se houver"
   ],
   "exemplos": [
     "exemplo retirado ou inspirado diretamente do capítulo",
     "outro exemplo relevante"
   ],
   "aplicacoes": [
     "como o estudante pode aplicar o que este capítulo ensina",
     "uma situação prática específica ligada ao conteúdo deste capítulo"
   ],
   "resumoRapido": [
     "ponto-chave 1 do capítulo",
     "ponto-chave 2",
     "ponto-chave 3"
   ]
 },
 "analogias": [
   "uma analogia com algo do cotidiano que ajude a entender o capítulo"
 ],
 "ativacao": [
   "pergunta reflexiva 1 conectada a este capítulo",
   "pergunta 2"
 ],
 "quiz": {
   "pergunta": "pergunta de múltipla escolha sobre um ponto central deste capítulo",
   "alternativas": [
     "alternativa A",
     "alternativa B",
     "alternativa C"
   ...
   "corretaIndex": 1,
   "explicacao": "explique a resposta certa, referenciando explicitamente o capítulo"
 },
 "flashcards": [
   {"q": "pergunta direta sobre um conceito importante deste capítulo", "a": "resposta objetiva"},
   {"q": "outra pergunta de revisão", "a": "resposta objetiva"}
 ]
}`;
    ...
    // --------------------------------------------------------
    // RENDERIZAÇÃO DO PLANO (cards à direita)
    // --------------------------------------------------------
    ...
    async function fluxoTema(tema, nivel) {
      ...
      try {
        ...
        wizard.plano = planoNorm;
        renderPlanoResumo(planoNorm);
